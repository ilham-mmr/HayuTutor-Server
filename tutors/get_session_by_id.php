...
$stmt = $conn->prepare(
    "SELECT users.id, users.full_name, users.picture,  sessions.session_id, sessions.subject, sessions.duration, sessions.price, sessions.date, sessions.time, sessions.location FROM users INNER JOIN sessions ON users.id = sessions.tutor_id WHERE sessions.tutor_id=? AND sessions.isBooked != 1"
);
