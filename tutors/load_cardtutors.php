...
$stmt = $conn->prepare("SELECT users.id, users.full_name, users.picture, sessions.price, sessions.subject, sessions.location FROM users INNER JOIN sessions ON users.id = sessions.tutor_id GROUP BY users.id LIMIT 0, 15");
$stmt->execute();
