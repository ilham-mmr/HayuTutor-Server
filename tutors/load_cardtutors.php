...
$stmt = $conn->prepare("SELECT users.full_name, users.picture, user_tutor.location, sessions.price, sessions.subject FROM users INNER JOIN user_tutor ON users.id = user_tutor.user_id INNER JOIN sessions ON user_tutor.tutor_id = sessions.tutor_id LIMIT 0, 3");
$stmt->execute();
